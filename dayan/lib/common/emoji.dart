...
class EmojiPicker extends StatefulWidget {
  ...
  final Color _defaultBgColor = Colors.white;//Color.fromRGBO(242, 242, 242, 1);
  ...
  EmojiPicker({
    ...
    this.categoryIcons,
    this.buttonMode = ButtonMode.CUPERTINO,
...
class _EmojiPickerState extends State<EmojiPicker> {
  ...
  Widget build(BuildContext context) {
        ...
        for (var i = 0; i < recommendedPagesNum; i++) {
          ...
          recommendedPages.add(
              Container(
                ...
                child: ListView(
                  children: List.generate((recommendedEmojis.length/widget.columns).ceil(), (index) {
                    return GridView.count(
                      shrinkWrap: true,
                      primary: true,
                      crossAxisCount: widget.columns,
                      children: List.generate(widget.columns, (subIndex) {
                        var i = index * widget.columns+ subIndex;
                        if (recommendedEmojis.length > i) {
                          return Center(
                            child: CupertinoButton(
                              pressedOpacity: 0.4,
                              padding: EdgeInsets.all(0),
                              child: Center(
                                child: Text(recommendedEmojis[i].emoji, style: TextStyle(fontSize: 30),),
                              ),
                              onPressed: () {
    ...
    int smileyPagesNum = 1; //(widget._smileys.values.toList().length / (widget.rows * widget.columns)).ceil();

    List<Widget> smileyPages = new List();
    int smileyLength = widget._smileys.values.toList().length;
    ...
    smileyPages.add(
      Container(
        ...
        child: GridView.count(
          ...
          primary: false,
          crossAxisCount: widget.columns,
          children: List.generate((smileyLength/widget.columns).ceil() * widget.columns, (subIndex) {
            int i = subIndex;
            if (smileyLength > i) {
              String emojiTxt = widget._smileys.values.toList()[i];
              return Center(
                child: CupertinoButton(
                  pressedOpacity: 0.4,
                  padding: EdgeInsets.all(0),
                  child: Center(
                    child: isAvailable(emojiTxt) ? Text(emojiTxt, style: TextStyle(fontSize: 32),) : widget.unavailableEmojiIcon,
                  ),
                  onPressed: () {
                    // _Recommended recommended = recommendedEmojis[index + (widget.columns * widget.rows * i)];
                    // widget.onEmojiSelected(Emoji(name: recommended.name, emoji: recommended.emoji), widget.selectedCategory);
    ...
    return Column(
      children: <Widget>[
        ...
        //             [Category.RECOMMENDED, Category.SMILEYS,
        //              //Category.ANIMALS, Category.FOODS, Category.TRAVEL, Category.ACTIVITIES, Category.OBJECTS, Category.SYMBOLS, Category.FLAGS
        //              ],
        //             [recommendedPagesNum, smileyPagesNum
        //             // , animalPagesNum, foodPagesNum, travelPagesNum, activityPagesNum, objectPagesNum, symbolPagesNum, flagPagesNum
        //             ]
        //         ),
